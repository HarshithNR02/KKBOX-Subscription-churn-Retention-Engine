# app/pages/1_Churn_Prediction.py
...
def get_sample_users():
    # Only show users where model agrees with actual outcome
    low    = master[(master['churn_prob'] < 0.03) & (master['is_churn'] == 0)][['msno','segment','churn_prob','clv','is_churn']].head(25)
    medium = master[(master['churn_prob'] >= 0.05) & (master['churn_prob'] < 0.20)][['msno','segment','churn_prob','clv','is_churn']].head(25)
    high   = master[(master['churn_prob'] >= 0.20) & (master['churn_prob'] < 0.50) & (master['is_churn'] == 1)][['msno','segment','churn_prob','clv','is_churn']].head(25)
    critical = master[(master['churn_prob'] > 0.80) & (master['is_churn'] == 1)][['msno','segment','churn_prob','clv','is_churn']].head(25)
    # rest stays same
    ...
    sample['actual'] = sample['is_churn'].apply(lambda x: '✅ Churned' if x == 1 else '🔄 Retained')
    return sample
...
with col1:
    if st.button("🟢 Random Low Risk", use_container_width=True):
        st.session_state['selected_msno'] = master[master['churn_prob'] < 0.03]['msno'].sample(1).values[0]
        st.rerun()
with col2:
    if st.button("🟡 Random Medium Risk", use_container_width=True):
        st.session_state['selected_msno'] = master[(master['churn_prob'] >= 0.05) & (master['churn_prob'] < 0.20)]['msno'].sample(1).values[0]
        st.rerun()
with col3:
    if st.button("🟠 Random High Risk", use_container_width=True):
        st.session_state['selected_msno'] = master[(master['churn_prob'] >= 0.20) & (master['churn_prob'] < 0.50)]['msno'].sample(1).values[0]
        st.rerun()
with col4:
    if st.button("🔴 Random Critical Risk", use_container_width=True):
        st.session_state['selected_msno'] = master[master['churn_prob'] > 0.80]['msno'].sample(1).values[0]
        st.rerun()
...
display_cols = ['msno', 'risk_tier', 'churn_prob_pct', 'segment', 'clv_fmt', 'actual']
...
if user_id and user_id.strip():
    ...
    if user_id not in master['msno'].values:
        ...
        raw_prob = model.predict_proba(user_features[FEATURE_COLS])[:, 1][0]
        churn_prob = raw_prob
        risk_label, risk_color = get_risk_tier(churn_prob)
        ...
        actual_churn = int(user_row['is_churn'])
        ...
        with r4:
            st.metric("CLV", f"{clv:,.0f} TWD")
        with r5:
            st.metric(
                "Actual Outcome",
                "✅ Churned" if actual_churn else "🔄 Retained",
                delta="Model correct" if (churn_prob > 0.5) == actual_churn else "Model incorrect",
                delta_color="normal" if (churn_prob > 0.5) == actual_churn else "inverse"
            )
